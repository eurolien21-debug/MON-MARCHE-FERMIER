...
function FinaliserSheet({ open, onClose, panier, livraison, setLivraison, adresses, dernierPaiement, setDernierPaiement, onConfirmer }) {
  ...
  const items = Object.entries(panier).map(([id, q]) => ({ ...PRODUCTS.find((p) => p.id === id), q })).filter(Boolean);
...
function CommanderScreen({ panier, setPanier, onOuvrirCompte, onContinuer, profil }) {
  ...
  const recommander = (produits) => {
    // Exemple simplifié : "Recommander" remplit le panier avec le poulet et les œufs
    setPanier({ poulet: 20 });
  };
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-2">
        <p className="mb-2 text-xs font-black uppercase tracking-wider" style={{ color: "#8B5E34" }}>Recommander en 1 clic</p>
        <div className="mb-4 flex flex-col gap-2">
          {RECOMMANDE.map((r) => (
            <button key={r.nom} onClick={() => recommander(r)} className="flex items-center justify-between rounded-2xl border-2 bg-white px-4 py-3 text-left shadow-sm" style={{ borderColor: "#EEE3CE" }}>
              <div>
                <p className="text-sm font-black" style={{ color: "#2B2620" }}>{r.nom}</p>
                <p className="text-xs font-semibold" style={{ color: "#B8AC94" }}>{r.detail}</p>
              </div>
              <RotateCcw size={20} style={{ color: "#2F6B4F" }} strokeWidth={2.6} />
            </button>
          ))}
        </div>
...
function EspaceScreen({ profil, onOuvrirCompte, commandeEnCours, reclamations, onNouvelleReclamation, onSupport }) {
  const ETAPES = [
    ...
    >
      ...
      >
        ...
        <OnboardingSheet
          ...
          onTermine={() => setOnboardingOuvert(false)}
        />
        <FinaliserSheet
